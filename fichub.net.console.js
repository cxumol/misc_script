...
var fichubToWget = url => fetch(`https://fichub.net/api/v0/epub?q=${encodeURIComponent(url)}`).then(r => r.json()).then(data => {
  ...
  var baseFileName = new URL(fullEpubUrl).pathname.split('/').pop().split('-').slice(0, -1).join('-') + `_${data.meta?.chapters}chs_zal.epub`;
  var ans = `wget -O "${baseFileName}" "${fullEpubUrl}"`;console.log(ans);return ans;
});
var url = '';
fichubToWget(url).then(cmd => copy(cmd));
